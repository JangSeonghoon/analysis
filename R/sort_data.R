test=data.frame(start=c(30,35,37,45,52,76),
                last=c(33,36,43,50,73,80),
                fix=c("면우","면우","면좌","방우","뒤틀림","수평"),
                measure=c(-15.2,28.3,5,29.5,-13.2,14),
                measure1=c(-15.2,28.3,5,29.5,-13.2,14)
                )
...
sort_data=cmpfun(
  function(){
   setwd("C:/Program Files/R")
    ...
    print("1")
    # assign("con",eval(parse(text=paste0("c(",content,")"))))
    ...
    test<<-test
    print("2")
    # test=test %>% select(start,last,con)
    print("1")
...
library(tidyr)
spread(sort,content1,content2)
###############


a=data.frame(a=c(1,2,3,4),b=c(3,4,5,6))
b=data.frame(a=c(1,2))
cbind(a,b)
cbind(data.frame(a=c(1,2,3,4)),data.frame(a=c(1,2)),data.frame(c(4)))
